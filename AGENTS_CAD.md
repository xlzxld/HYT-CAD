...
| 出线槽脚本    | `cx_runner.lsp`（**v11.2**，1764 行，79 defun，命令 CX/CXPARAM，保留 c:SLOT 兼容别名；断口圆角函数 `dt:slot-fillet-pair`，CXK 图层亮蓝 150；**生成出线槽时自动布置压线板**(YXB 图层青色 4, 模板=v11.2 新 D 形 6 件(重合线16.6/上下边11/右边15.3/R4.3弧×2, 出自更新后 tools\1.dxf), 重合线中点(白线交界)=定位点, 沿选定壁 gap 均匀分布整组居中+斜壁旋转, plan/place 两步——删源线前规划/删源线后放置, 无碰撞判断, find-wall 按层取实体(entnext 全库扫描死循环已根治)；全文件零 (command) 主流程(COM标记撤销, 坑 #69 根除)） |
| 加热条脚本    | `jrt_runner.lsp`（**v9.24**，2708 行，111 defun，多模板 通用一/通用二，命令 JRT/JRTPARAM；通用二支持**单线自动补边**——外壁整圈 + JRTDW 标记出线口, 出线口自动开出；**方向几何判定(v9.24)**: 嵌套内偏恒朝轮廓围合区域内侧/颈线恒朝 FLB 外侧, 与 JRTDW 画向无关, 歧义回退 JRTDW 就近；坑 #73 已根治(undo-mark 缺括号吞函数, hook-one 等回顶层)；全文件零 (command) 调用, 坑 #69 根除) |

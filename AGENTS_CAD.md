...
| 出线槽脚本    | `cx_runner.lsp`（**v10.4**，1498 行，70 defun，命令 CX/CXPARAM，保留 c:SLOT 兼容别名；断口圆角函数 `dt:slot-fillet-pair`，CXK 图层亮蓝 150）                                                                                                                                                                                                                                                   |
| 加热条脚本    | `jrt_runner.lsp`（**v9.15**，2413 行，95 defun，多模板 通用一/通用二，命令 JRT/JRTPARAM；通用二支持**单线自动补边**——只画 JRTDW + 一条最外侧壁线, 对侧壁线与两端喇叭自动生成) |
| 尺寸测量与外协 | `wx_runner.lsp`（**v2.2**，1395 行，49 defun，独立第 4 脚本，命令 FLBSZ/XQG/JD/SJTZ；工件倾斜自动正交摆正；精雕原生活动文档正反面镜像排版[间隔75mm]，手动模式100%全保留/自动模式分流排版；主进胶ZJJ与DP白名单提取；数据图纸独立图层隔离防测量干扰；COM原生撤销标与无stringp兼容） |
...
| jrt | **v9.15** | **通用二「单线自动补边」(jrt_runner.lsp v9.15, 2413行, 95 defun)**：条内源线仅 1 段且为直线(LINE)时自动合成闭合外轮廓——①对侧壁线 = 该壁线朝 JRTDW 方向偏 2×半宽(跨过定位线落到对侧同距处, 复用 dt:jrt2-cands + dt:jrt2-pick-near 选向)；②两端各生成 R 喇叭过渡弧×2 + 加宽平端线(端部总宽 = 条宽 + 2R, 壁线端点即喇叭切点, 旧流程的"手动裁剪+圆角"整体免掉)；③新参数 `jrt2_half_w`(16.5)/`jrt2_end_r`(12) 进参数框/ini/记忆全链路(模板键表驱动)；④生成件并入句柄记忆(重跑先删, 源壁线保留不改动)；⑤多段源线(手画整轮廓)走旧流程零变化；壁线距定位线与半宽偏差超 1.0 警告, 非直线壁线提示回退。来源: demo_recorder 操作日志反推端部几何(与用户手画弧圆心/半径/起止角逐参数吻合), 端部结构经用户确认 |

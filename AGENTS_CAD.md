...
| 加热条脚本 | `jrt_runner.lsp`（**v9.37**，3092 行，119 defun，多模板 通用一/通用二，命令 JRT/JRTPARAM；**v9.37 真 2007 专修(坑 #74 终局)**: c:JRT 原流程"模板框→参数框"两连开, 模板框关闭瞬间触发菜单 Automation 错误风暴打断命令(仅通用二必现; 直连 (dt:jrt2-process) 实测两次全部出图正常) → 模板单选合并进参数对话框(单开一个 DCL, 非当前模板参数置灰), c:JRT 不再打开模板选择框, 模板记忆不变；；**封闭线定位层 JRTFBX(浅橙 21)**——v9.36 定案: 通用二破口封闭线**本体在 JRT**(加热条结构完整, 与 v9.33 一致), "JRTFBX" 仅为其同几何定位副本(dt:jrt2-trace, 供另一项目建模脚本按层定位; 不在外协白名单, 不进数据图纸/精雕出图, 重跑随产物自清)；通用二支持**单线自动补边**——外壁整圈 + JRTDW 定位线(长线一头埋板内亦可, 两端按轮廓围合区域分里外; 通道线裁板内侧留板边侧+外端延长保留), 出线口自动开出, **圆弧/圆外壁通道线双端自动延伸(参数 jrt2_hook_ext 默认5, 0=旧行为)**；**方向几何判定(v9.25/26/27)**: 嵌套内偏=整环传播 A/B 两候选环取离 FLB 壁更远者朝内(围合奇偶→JRTDW 逐级回退)/颈线=FLB 围合区 24 段采样奇偶判外(L 形非凸也对, 包围盒中心兜底)；vlax-curve nil 返回全双判(numberp: nil 崩溃已根治)；坑 #73 已根治(undo-mark 缺括号吞函数, hook-one 等回顶层)；全文件零 (command) 调用, 坑 #69 根除) |
...
| 引导器 | `dt_start.lsp`（**v3.6**，815 行，31 defun；一键加载+随 CAD 自启动+顶部菜单「热流道自动化(R)」(二级序: 分流板/加热条/出线槽/外协加工/工具)+**boot/DTRELOAD 逐家族加载验证**；v2.9 起含 `dt:st-gets`/`dt:st-hasvar`/`dt:st-support-root` 低版本兼容底座。**2026-09-19 坑 #74/#75 处置决定(用户定案): 本轮针对「JRT 通用二报错」的菜单侧修改(MENULOAD 文件菜单/COM Load/命令面板)全部回滚到 v3.6 —— 用户证实菜单在改动前挂载与使用均正常, 不应改动; 唯一保留: dt:st-menu-remove 末尾追加一次性清理(MENUGROUP 检测到 DTTOOLS 即 MENUUNLOAD), 卸掉修复期间误装入的残菜单; 通用二本体问题转入 `(dt:jrt2-process)` 直连隔离定位, 见坑 #74 末尾处置记录) |
...
|---|---|---|---|---|
| **2007**（32位，精简版） | ✅ **须用 scripts_ansi\ 的 GBK 副本** | ✅ | ✅ 菜单挂载与使用正常（用户证实; `MenuGroups.Add` 报「未知名称: Add」→ 走 ACAD 主菜单组回退, 该状态实测可用）。注意: 本机连开两个 DCL 对话框会触发菜单 Automation 错误风暴打断命令 —— 已由 jrt v9.37 单对话框化根治(坑 #74) | **已实测**（2026-08-31 安装/自启/全命令；2026-09-19 JRT 通用二修复并验证） |
...
>
> **顶部菜单在 2007 精简版上的说明（坑 #74/#75 处置定案 2026-09-19）**：
> 该机 `MenuGroups.Add` 报「未知名称: Add」（真 2007 有该方法，报错即精简版）→ 走"popup 塞进 ACAD 主菜单组"回退，
> **该状态实测挂载与使用均正常（用户证实）**。曾误判"COM popup 在菜单栏刷新时抛错打断命令"并为此做了
> MENULOAD 文件菜单/COM Load/命令面板等一整轮菜单侧改造（v3.13~v3.16），实测证实**菜单侧任何改造都会引入新问题，
> 且都不是 JRT 通用二报错的根因** —— 已于 2026-09-19 整体回滚到 v3.6 菜单逻辑（用户验证过的状态），
> 唯一保留 dt:st-menu-remove 末尾对修复期间误装的 DTTOOLS 部分菜单组的 MENUUNLOAD 清理。
> JRT 通用二报错的真正根因与修复见坑 #74 正文与 jrt v9.37（对话框连开两段的时序问题，与菜单无关）。
> 教训：**用户说"只有一个问题"时，改动面必须锁死在该问题上**；周边系统"看起来有问题"时应先用直连实验证实，再动手。
...
74. **`(c:JRT)` 选「通用二」报菜单 Automation 错误风暴打断命令 —— 根因 = "模板框→参数框"两连开 DCL 的特定时序，jrt v9.37 单对话框化根治**（2026-09-19 实测定案，用户确认修复）：
   - **原始现场**：菜单/命令行跑 `(c:JRT)` 选「通用二」，模板横幅打印到一半即报
     `Automation 错误。参数 热流道自动化(&R) (位于 Item 中) 无效`（另见 `无效索引`/`未知名称: Detach`/
     `发生未知意外` 交织），命令死亡；FLB / CX / 通用一 全部正常。vl-catch-all 抓不住。
   - **弯路（引以为戒）**：报错点名菜单标题 ⇒ 连续四轮把矛头指向菜单（COM popup 换文件菜单/COM Load/
     不建菜单/命令面板，jrt v9.36 + dt_start v3.13~v3.16），越改越糟 —— 直到用户直连
     `(dt:jrt2-process)` **两次全部成功出图**，才把二分做完：**通用二几何本体零问题，病灶在
     "模板框关闭→参数框打开"的对话框切换时序**；且该机菜单在改动前本就正常（用户证实），
     菜单侧任何改造（含"已加载未挂顶栏的部分菜单"）都会制造新的打断源。
   - **根因（表述留白）**：真 2007 精简版上，模板 DCL 关闭→参数 DCL 打开的两连开时序会触发
     菜单 Automation 错误风暴（错误名随当时菜单状态变化）；单开一个对话框实测安全。
     为什么仅通用二必现（FLB 两连开正常）尚未完全解释 —— 不影响修复有效性。
   - **修复（jrt v9.37）**：模板单选合并进参数对话框（jrt_param 增 boxed_radio_column 模板列 +
     全部 13 参数平铺、非当前模板参数 mode_tile 置灰；dt:jrt-tpl-switch/tpl-mode 切换；
     dt:jrt-param-apply 先落 *jrt-template*），c:JRT 不再打开模板选择框；dt:jrt-template-dialog /
     dt:jrt-template-dcl-lines 删除；模板记忆不变（mem [模板] template=下标，下次自动选中）。
   - **配套**：dt_start 回滚 v3.6 + DTTOOLS 残菜单 MENUUNLOAD 清理；新增结构门禁 check_sexpr.py（坑 #75）。
   - **验证（用户 2007 实机）**：v3.14 起加载恢复正常（坑 #75 修复）；v3.16+DT 面板下 JRT 已可用；
     v9.37 合并对话框后用户确认"可以了"。
   - **教训**：①用户圈定的问题范围就是改动边界，周边"疑似病灶"先用直连实验证实再动；
     ②绕开中间层直连核心逻辑（如 `(dt:jrt2-process)`）一次就能完成二分 —— 本应第一时间做；
     ③错误信息点名谁，谁未必是根因（它只是刷新时最先被撞上的对象）。

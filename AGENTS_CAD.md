...
| 主脚本(分流板) | `flb_runner.lsp`（**v10.8**，2620 行，101 defun，多模板 通用/矩形，命令 FLB/FLBPARAM，保留 c:OFF 兼容别名；假体默认传统逐步, 参数框勾选「包络法」切换；新增参数「主进胶R (zjj_r 14.35)」与图层 ZJJ，自动扫描 DP 圆心生成主进胶圆；14 层颜色调优去重) |
| 出线槽脚本    | `cx_runner.lsp`（**v11.3**，1831 行，80 defun，命令 CX/CXPARAM，保留 c:SLOT 兼容别名；断口圆角函数 `dt:slot-fillet-pair`，CXK 图层亮蓝 150；**生成出线槽时自动布置压线板**(YXB 图层青色 4, 模板=v11.2 新 D 形 6 件(重合线16.6/上下边11/右边15.3/R4.3弧×2, 出自更新后 tools\1.dxf), 重合线中点(白线交界)=定位点, 沿选定壁 gap 均匀分布整组居中+斜壁旋转, **壁候选=直线+多段线直段(v11.3), 弧段跳过+失败自诊断计数**, plan/place 两步——删源线前规划/删源线后放置, 无碰撞判断, entnext 全库扫描死循环已根治)；全文件零 (command) 主流程(COM标记撤销, 坑 #69 根除)） |
| 加热条脚本    | `jrt_runner.lsp`（**v9.26**，2878 行，116 defun，多模板 通用一/通用二，命令 JRT/JRTPARAM；通用二支持**单线自动补边**——外壁整圈 + JRTDW 标记出线口, 出线口自动开出, **圆弧/圆外壁通道线双端自动延伸(参数 jrt2_hook_ext 默认5, 0=旧行为)**；**方向几何判定(v9.25/26)**: 嵌套内偏=整环传播 A/B 两候选环取离 FLB 壁更远者朝内(围合奇偶→JRTDW 逐级回退)/颈线=FLB 围合区 24 段采样奇偶判外(L 形非凸也对, 包围盒中心降为兜底)；vlax-curve nil 返回全双判(numberp: nil 崩溃已根治)；坑 #73 已根治(undo-mark 缺括号吞函数, hook-one 等回顶层)；全文件零 (command) 调用, 坑 #69 根除) |
| 尺寸测量与外协 | `wx_runner.lsp`（**v2.8**，1593 行，52 defun，独立第 4 脚本，命令 FLBSZ/XQG/JD/SJTZ；工件倾斜自动正交摆正；**精雕镜像在本体右侧(竖直轴,间隔75mm)，每幅(本体+镜像+文字)绿色包络盒框住(实体级颜色,盒=内容精确bbox)；精雕/线切割 4 幅一行网格排版(游标=纯几何推导:"外协文字"层MText为单位标记, 行内x锚定当前行文字bbox maxx+box_gap, 弃跨文档sysvar)；盒间距统一 ini [排版] box_gap=100；剪贴板格式 350*180**；主进胶ZJJ与DP白名单提取；数据图纸独立图层隔离防测量干扰；全文件零 (command) 主流程(COM标记撤销)） |
| 引导器      | `dt_start.lsp`（**v3.2**；一键加载+随 CAD 自启动+**顶部菜单「热流道自动化(R)」**(二级序: 分流板/加热条/出线槽/外协加工/工具, 工具内含演示记录器 DTDEMO 按需加载 demo_recorder.lsp)+**boot/DTRELOAD 逐家族加载验证**(主命令缺失=半加载, 明确警告)；v2.9 起含 `dt:st-gets`/`dt:st-hasvar`/`dt:st-support-root` 低版本兼容底座，**全版本通用**（2026-08-31 AutoCAD 2007 实测通过）。发版后无独立 old 副本） |
| 演示记录器   | `demo_recorder.lsp`（**v1.0**，277 行，14 defun，开发辅助工具：命令级反应器把手工操作录成 AI 可读日志；菜单 DTDEMO 按需加载，命令 DEMOREC/DEMOSTOP/DEMOMARK） |

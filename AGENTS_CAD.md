...
| 加热条脚本    | `jrt_runner.lsp`（**v9.25**，2818 行，114 defun，多模板 通用一/通用二，命令 JRT/JRTPARAM；通用二支持**单线自动补边**——外壁整圈 + JRTDW 标记出线口, 出线口自动开出；**方向几何判定(v9.25)**: 嵌套内偏=整环传播 A/B 两候选环取离 FLB 壁更远者朝内(围合奇偶→JRTDW 逐级回退)/颈线恒朝 FLB 包围盒外侧, 与 JRTDW 画向无关；vlax-curve nil 返回全双判(numberp: nil 崩溃已根治)；坑 #73 已根治(undo-mark 缺括号吞函数, hook-one 等回顶层)；全文件零 (command) 调用, 坑 #69 根除) |

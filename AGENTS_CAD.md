...
| 引导器 | `dt_start.lsp`（**v3.16**，1105 行，35 defun；一键加载+随 CAD 自启动+顶部菜单(二级序: 分流板/加热条/出线槽/外协加工/工具)+**boot/DTRELOAD 逐家族加载验证**(主命令缺失=半加载, 明确警告)；v2.9 起含 `dt:st-gets`/`dt:st-hasvar`/`dt:st-support-root` 低版本兼容底座。**版本路线**: v3.13 坑 #74 —— 用户 2007 为精简版(MenuGroups.Add 报"未知名称: Add"), COM popup 在菜单栏刷新时抛抓不住的 Automation 错误打断命令 → Add 失败且 ACADVER<24 改走 MENULOAD 文件菜单(`dt:st-menu-fileload`, 不碰 ActiveX)；v3.14 坑 #75 —— v3.13 的 `dt:st-filemenu-lines` then 分支 `(setq` 少一个右括号(else 被吞, 末行多一个 `)` 补平总数, 平衡类门禁全查不出, 真 2007 `(load)` 报「语法错误」)→ 补括号修复 + **新增门禁 `tools/check_sexpr.py`**(setq 奇偶/if/foreach 元素数)；v3.15 —— 真机实测 MENULOAD 的错误会异步浮现(下一次 LISP 调用才抛)打断挂载 → 菜单加载两路线(A=COM MenuGroups.Load 优先/B=MENULOAD+menugroup 探测吸异步错)；**v3.16 —— 真机再实测: 连"已加载未挂顶栏的部分菜单"都会在刷新时打断命令(JRT 选通用二即死) → 精简机上不加载任何菜单(启动自动 MENUUNLOAD 清理), 新增 c:DT 命令面板(DCL 点选执行全部命令, 不碰菜单引擎) + c:DTMENU(手动试挂, 自担风险)；真版(2021+/Add 成功)行为不变, 顶栏菜单照旧) |

...
| 加热条脚本    | `jrt_runner.lsp`（**v9.35**，3086 行，119 defun，多模板 通用一/通用二，命令 JRT/JRTPARAM；**封闭线独立图层 JRTFBX(浅橙 21, v9.34, 仅通用二)**——通用二破口封闭线(即加热条出线口封闭线)放该层, 通用一帽线于 v9.35 回退至 JRT(通用一为嵌套轮廓半成品, 无出线口, 其帽线属端帽), 通用二重跑幂等清理含 JRTFBX；通用二支持**单线自动补边**——外壁整圈 + JRTDW 定位线(长线一头埋板内亦可, 两端按轮廓围合区域分里外; 通道线裁板内侧留板边侧+外端延长保留), 出线口自动开出, **圆弧/圆外壁通道线双端自动延伸(参数 jrt2_hook_ext 默认5, 0=旧行为)**；**方向几何判定(v9.25/26/27)**: 嵌套内偏=整环传播 A/B 两候选环取离 FLB 壁更远者朝内(围合奇偶→JRTDW 逐级回退)/颈线=FLB 围合区 24 段采样奇偶判外(L 形非凸也对, 包围盒中心兜底)；vlax-curve nil 返回全双判(numberp: nil 崩溃已根治)；坑 #73 已根治(undo-mark 缺括号吞函数, hook-one 等回顶层)；全文件零 (command) 调用, 坑 #69 根除) |
...
| JRTFBX | 加热条封闭线 | 浅橙 21 | v9.34：通用二破口封闭线（=加热条出线口封闭线）放此层；v9.35 通用一帽线已回退至 JRT（通用一无出线口；OFF 预建, 通用二画并重跑自清） |

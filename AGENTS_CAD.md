...
| 出线槽脚本    | `cx_runner.lsp`（**v10.5**，1761 行，80 defun，命令 CX/CXPARAM，保留 c:SLOT 兼容别名；断口圆角函数 `dt:slot-fillet-pair`，CXK 图层亮蓝 150；**生成出线槽时自动布置压线板**(YXB 图层青色 4, 模板出自 tools\1.dxf, 贴壁+斜壁旋转+碰撞滑动让位+运行时选左/右壁)；全文件零 (command) 主流程(COM标记撤销, 坑 #69 根除)） |
| 加热条脚本    | `jrt_runner.lsp`（**v9.17**，2561 行，104 defun，多模板 通用一/通用二，命令 JRT/JRTPARAM；通用二支持**单线自动补边**——外壁整圈 + JRTDW 定位短线, 出线口自动开出；**全文件零 (command) 调用**, 坑 #69 根除) |
| 尺寸测量与外协 | `wx_runner.lsp`（**v2.3**，1471 行，51 defun，独立第 4 脚本，命令 FLBSZ/XQG/JD/SJTZ；工件倾斜自动正交摆正；**精雕镜像在本体右侧(竖直轴,间隔75mm)，每幅(本体+镜像+文字)绿色包络盒框住(图层"外协包络盒")；精雕/线切割 4 幅一行网格排版(行满向下追加,列距/行距 ini [排版] 可调,游标存目标图 USERI5/USERR5)**；主进胶ZJJ与DP白名单提取；数据图纸独立图层隔离防测量干扰；全文件零 (command) 主流程(COM标记撤销)） |
| 引导器      | `dt_start.lsp`（**v3.1**，787 行，31 defun；一键加载+随 CAD 自启动+**顶部菜单「热流道自动化(R)」**(二级序: 分流板/加热条/出线槽/外协加工/工具, 工具内含演示记录器 DTDEMO 按需加载 demo_recorder.lsp)；v2.9 起含 `dt:st-gets`/`dt:st-hasvar`/`dt:st-support-root` 低版本兼容底座，**全版本通用**（2026-08-31 AutoCAD 2007 实测通过）。发版后无独立 old 副本） |
...
| JRTDW | 加热条定位 | 橙黄 40 | 通用二加热条的定位源线（用户画） |
...
| cx | **v10.5** | **出线槽自动布置压线板 (cx_runner.lsp v10.5, 1761行, 80 defun; 用户需求, 模板=tools\1.dxf 的 YXB 图层)**：①c:CX 运行时询问贴左壁/右壁(initget L/R)；②在 dt:cx-process 的封口与删源线之间执行 dt:cx-yxb——每条源线选定侧的直壁上，每隔 cx_yxb_gap(125, 进参数框/ini)放置一幅压线板；③模板 9 实体(重合线 16.6 贴壁 + 上下边 11 + 右端 R4 弧×2 + 中部 R5.75 圆 + 2 条 0.01 碎线；1.dxf 中重复弧只画一次)，重合线与壁线完全重合(斜壁同步旋转, 本地 +Y→壁方向 / +X→外侧法向)；④碰撞规则：重合线以外的曲线与图中任何实体相交即碰撞，沿壁滑动让位(步进 10, 上限 50)，无位可让跳过该处；⑤YXB 层青色 4 脚本独占(每次运行先清旧实例)；钩子在删源线之前执行(用源线判外侧方向)；⑥坑 #69 根除：本文件 13 处 (command "_.UNDO") 改 dt:cx-undo-mark/-end(COM 标记)，*error* 不再碰 (command)；check_lisp CX 白名单同步(dt:cx-yxb 系列 7 函数, 并修正历史死条目 dt:cx-break-curve→dt:break-curve) |

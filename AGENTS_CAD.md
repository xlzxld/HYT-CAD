...
| 加热条脚本    | `jrt_runner.lsp`（**v9.34**，3086 行，119 defun，多模板 通用一/通用二，命令 JRT/JRTPARAM；**封闭线独立图层 JRTFBX(浅橙 21, v9.34)**——通用一直线帽的帽线/整弧替换帽与通用二破口封闭线放该层, 圆帽圆与帽角过渡弧仍在 JRT, 重跑幂等清理含 JRTFBX, snapshot/diff 改两层并集；通用二支持**单线自动补边**——外壁整圈 + JRTDW 定位线(长线一头埋板内亦可, 两端按轮廓围合区域分里外; 通道线裁板内侧留板边侧+外端延长保留), 出线口自动开出, **圆弧/圆外壁通道线双端自动延伸(参数 jrt2_hook_ext 默认5, 0=旧行为)**；**方向几何判定(v9.25/26/27)**: 嵌套内偏=整环传播 A/B 两候选环取离 FLB 壁更远者朝内(围合奇偶→JRTDW 逐级回退)/颈线=FLB 围合区 24 段采样奇偶判外(L 形非凸也对, 包围盒中心兜底)；vlax-curve nil 返回全双判(numberp: nil 崩溃已根治)；坑 #73 已根治(undo-mark 缺括号吞函数, hook-one 等回顶层)；全文件零 (command) 调用, 坑 #69 根除) |
| 尺寸测量与外协 | `wx_runner.lsp`（**v2.11**，1646 行，51 defun，独立第 4 脚本，命令 FLBSZ/XQG/JD/SJTZ；工件倾斜自动正交摆正；**精雕镜像在本体右侧(竖直轴,间隔75mm)，每幅(本体+镜像+文字)紫红包络盒框住(实体级颜色,盒=内容精确bbox)；精雕/线切割 4 幅一行网格排版(游标=纯几何推导:"外协文字"层MText为单位标记, 行内x锚定当前行文字bbox maxx+box_gap, 弃跨文档sysvar)；盒间距统一 ini [排版] box_gap=100；剪贴板格式 350*180**；主进胶ZJJ与DP白名单提取；数据图纸独立图层隔离防测量干扰；v2.11 配色重排(FLB_BOX 赭黄42/数据图纸 橄榄绿63/外协文字 深青144/JD 薄荷绿101/外协包络盒 紫红222)；全文件零 (command) 主流程(COM标记撤销)） |
...
| 回归门禁     | `tools\test_direction.py`（**16 组 89 断言**, 纯 stdlib: 点内判定/封口/颈向/压线板排布与模板/镜像弧角闭合(v11.7)/内外壁取侧(v11.7), 口径锚定 tools\1.dxf·2.dxf 实测）+ `tools\check_audit_fixes.py`（13 项体检修复回归）+ `tools\check_defun_depth.py`（defun 顶层深度门禁）+ `tools\check_layer_colors.py`（图层配色门禁: 全脚本登记色一致/唯一/两两 Lab 色差 ≥30, 伴生 `tools\_aci_colors.py` 标准色表）+ `tools\check_jrt2_out.py`（JRT 通用二产物检查, 需 AutoCAD 出图后人工执行; 口径=1714 测试图, 已移出仓库; v9.34 起默认查 JRT+JRTFBX 并集）；均带退出码阻断, 命令清单见根目录 AGENTS.md §2 |
...
| JRTFBX | 加热条封闭线 | 浅橙 21 | v9.34：通用一帽线/整弧替换帽 + 通用二破口封闭线放此层（圆帽圆与帽角过渡弧仍在 JRT；OFF 预建, jrt 画并重跑自清） |
...
| JD | 精雕合并层 | 薄荷绿 101 | v2.8 精雕正反面合并单层, ByLayer 实体先转存原层色再换层 |
| 外协包络盒 | 精雕包络盒 | 紫红 222 | v2.3 精雕每幅(本体+镜像+文字)整体包络盒(实体级同色) |

> `ensure-layer`（flb/jrt/wx）对已存在但大小写不一致的图层自动改名纠正（如 dp→DP；AutoCAD 图层名不区分大小写，坑 #29）。CXK/YXB 由 cx 内联创建（无 ensure-layer）。**v10.11 起全 21 图层统一配色重排**（旧版 5 对图层完全同色、黄色/蓝色系大面积相近, 重排后两两最小 Lab 色差 31.3），且 ensure-layer 对已存在图层也把颜色校正为登记值（老图重跑自动换新色）；`tools\check_layer_colors.py` 门禁防多脚本分头写色漂移。

...
| 出线槽脚本    | `cx_runner.lsp`（**v11.0**，1730 行，79 defun，命令 CX/CXPARAM，保留 c:SLOT 兼容别名；断口圆角函数 `dt:slot-fillet-pair`，CXK 图层亮蓝 150；**生成出线槽时自动布置压线板**(YXB 图层青色 4, 模板出自 tools\1.dxf, 贴壁+斜壁旋转, plan/place 两步——删源线前规划/删源线后无条件均匀放置, 无碰撞判断)；全文件零 (command) 主流程(COM标记撤销, 坑 #69 根除)） |
| 加热条脚本    | `jrt_runner.lsp`（**v9.22**，2585 行，105 defun，多模板 通用一/通用二，命令 JRT/JRTPARAM；通用二支持**单线自动补边**——外壁整圈 + JRTDW 定位短线, 出线口自动开出；坑 #73 已根治(undo-mark 缺括号吞函数, hook-one 等回顶层)；全文件零 (command) 调用, 坑 #69 根除) |

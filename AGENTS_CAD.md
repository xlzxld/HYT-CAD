...
| 出线槽脚本    | `cx_runner.lsp`（**v10.6**，1768 行，80 defun，命令 CX/CXPARAM，保留 c:SLOT 兼容别名；断口圆角函数 `dt:slot-fillet-pair`，CXK 图层亮蓝 150；**生成出线槽时自动布置压线板**(YXB 图层青色 4, 模板出自 tools\1.dxf, 贴壁+斜壁旋转+碰撞滑动让位+运行时选左/右壁; 碰撞检测实例级 bbox 预过滤)；全文件零 (command) 主流程(COM标记撤销, 坑 #69 根除)） |
| 加热条脚本    | `jrt_runner.lsp`（**v9.18**，2568 行，104 defun，多模板 通用一/通用二，命令 JRT/JRTPARAM；通用二支持**单线自动补边**——外壁整圈 + JRTDW 定位短线, 出线口自动开出；**文件尾加载完整性自检 + c:JRT 入口版本守卫**（半加载明确提示）；**全文件零 (command) 调用**, 坑 #69 根除) |
| 尺寸测量与外协 | `wx_runner.lsp`（**v2.4**，1496 行，50 defun，独立第 4 脚本，命令 FLBSZ/XQG/JD/SJTZ；工件倾斜自动正交摆正；**精雕镜像在本体右侧(竖直轴,间隔75mm)，每幅(本体+镜像+文字)绿色包络盒框住(实体级颜色,图层"外协包络盒")；精雕/线切割 4 幅一行网格排版(行满向下追加,列距/行距 ini [排版] 可调; 游标=纯几何推导: "外协文字"层 MText 为单位标记, 弃跨文档 sysvar)**；主进胶ZJJ与DP白名单提取；数据图纸独立图层隔离防测量干扰；全文件零 (command) 主流程(COM标记撤销)） |
